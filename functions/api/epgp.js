...
export async function onRequest({ request, env }) {
  ...
  if (request.method === 'GET') {
    try {
      ...
      const settingKeys = ['min_vault_level', 'vault_1_ep', 'vault_2_ep', 'vault_3_ep', 'signup_ep', 'on_time_ep', 'on_time_reason', 'default_gp'];
      const vaultSettings = {};
    ...
    try {
      ...
      if (vault_settings && typeof vault_settings === 'object') {
        ...
        for (const [key, value] of Object.entries(vault_settings)) {
          if (['min_vault_level', 'vault_1_ep', 'vault_2_ep', 'vault_3_ep', 'signup_ep', 'on_time_ep', 'on_time_reason', 'default_gp'].includes(key)) {
